...
create table `DBSWITCH_ASSIGNMENT_CONFIG` (
  ...
  `target_always_drop`          tinyint(1)            not null default 0      comment '是否每次都删除重建（完全覆盖模式）',
  `smart_recovery`              tinyint(1)            not null default 0      comment '是否启用智能容错1+2模式（0否1是）',
  `recovery_state`              tinyint(1)            not null default 0      comment '智能容错状态（0正常态1待恢复态）',
  `consec_fail_count`           int                   not null default 0      comment '连续失败次数',
